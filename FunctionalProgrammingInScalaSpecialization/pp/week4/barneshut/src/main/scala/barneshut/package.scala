...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        bodies.foreach(bd => fork = fork.insert(bd))
        fork.insert(b)
      } else {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      if (xb > boundaries.maxX) {
        xb = boundaries.maxX
      } else if (xb < boundaries.minX) {
        xb = boundaries.minX
      }

      if (yb > boundaries.maxY) {
        yb = boundaries.maxY
      } else if (yb < boundaries.minY) {
        yb = boundaries.minY
      }

      val x: Int = ((xb - boundaries.minX) / sectorSize).toInt
      val y: Int = ((yb - boundaries.minY) / sectorSize).toInt
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      for (i <- 0 until matrix.length) matrix(i) = matrix(i).combine(that(i % sectorPrecision, i / sectorPrecision))
      this
    }
