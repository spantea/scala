...
import FloatOps._
  ...
  test("Insert in leaf with size > minSize") {
    val b = new Body(123f, 2.5f, 2.5f, 0f, 0f)
    val leaf = Leaf(5.0f, 5.0f, 10, Seq())
    val quad = leaf.insert(b)
    quad match {
      case Fork(nw, _, _, _) => assert(nw.total == 1)
      case _ => assert(false)
    }
  }
