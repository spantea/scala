...
class KMeansSuite extends FunSuite {
  ...
  def checkUpdate(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point], expected: GenSeq[Point]) {
    assert(update(classified, oldMeans) == expected,
      s"classify($classified, $oldMeans) should equal to $expected")
  }

  test("'update should work for empty 'classified points' and empty 'old means'") {
    val classified: GenMap[Point, GenSeq[Point]] = GenMap[Point,GenSeq[Point]]()
    val means: GenSeq[Point] = IndexedSeq()
    val expected = IndexedSeq()
    checkUpdate(classified, means, expected)
  }
